...
CHAN_SKINS_SUBMIT = 985717921600929872
...
def check_if_staff(message: discord.Message):
    ...
    return message.guild is None or message.guild.id != GUILD_DDNET \
           or message.channel.id != CHAN_SKINS_SUBMIT or is_staff(author)


def check_if_has_attachments(message: discord.Message):
    if len(message.attachments) == 0:
        return (False, 'Your submission is missing attachments. Attach all skins to one message and '
                       'follow the correct message format written in <#986941590780149780>')
    return True, None
...
def check_image_resolution(message: discord.Message):
    for attachment in message.attachments:
        if (attachment.height != 128 or attachment.width != 256) and (attachment.height != 256 or attachment.width != 512):
            return (False, 'One of the attached skins does not have the correct image resolution. '
                           'Resolution must be 256x128, and if possible provide a 512x256 along with the 256x128')
    return True, None
...
def check_text_format(message: discord.Message):
    ...
    if not re_match:
        return (False,
                'Your message isn\'t properly formatted. Follow the message format written in <#986941590780149780>. '
    ...
    if re_match.group('license') is not None:
        if not any(lisense in re_match.group('license') for lisense in licenses):
            return (False, 'Wrong License. Possible licenses: `(CC0)`, `(CC-BY)` or `(CC-BY-SA)`'
                           ...
                           '\nCC-BY-SA - skin can be used/edited/shared, '
                           'but must be credited and derived works must also be shared under the same license```')
    return True, None
